![header](https://capsule-render.vercel.app/api?type=Waving&text=misung&color=fafaf9&fontColor=ffffff&fontAlignY=35&height=200)

<div align="left">
  <h1>👋 Hello! I'm misung 🇰🇷🇯🇵</h1>
  <p>
    Experienced <b>Backend Engineer</b> currently based in <b>Osaka, Japan</b>.<br>
    Proven leadership experience as a Team Leader in a startup environment.
  </p>

  <br>

  <h2>🏢 Work Experience</h2>
  <ul>
    <li><b>2026.04 ~ Present</b>: Backend System Engineer @ <b>Upload Inc. (Osaka, Japan)</b></li>
    <li><b>2024.07 ~ 2025.12</b>: Full Stack <b>Team Leader</b> @ <b>NeuroCircuit</b> (NestJS, Flutter)</li>
    <li><b>2022.11 ~ 23.11</b>: Software Engineer @ <b>Metanet Digital/Global</b> (Spring, SAP ABAP)</li>
  </ul>

  <br>

<h2>📜 Certifications & Language</h2>
  <table width="100%">
    <thead>
      <tr>
        <th width="45%">Certification / Skill</th>
        <th width="30%">Issued by</th>
        <th width="25%">Status / Date</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td>
          <img width="20" src="https://github.com/user-attachments/assets/c1c1dd93-e4d3-4d90-9bdc-9bb517d2d59c" style="vertical-align: middle;" /> 
          <b>Java SE 11 Developer (Silver)</b>
        </td>
        <td>Oracle</td>
        <td><b>Passed</b> (Feb 2026)</td>
      </tr>
      <tr>
        <td><b>JLPT N1 (Japanese)</b></td>
        <td>Japan Foundation</td>
        <td><i>Target: July 2026</i></td>
      </tr>
      <tr>
        <td><b>JLPT N2 (Japanese)</b></td>
        <td>Japan Foundation</td>
        <td>Passed (2014)</td>
      </tr>
      <tr>
        <td><b>AWS Solutions Architect (SAA)</b></td>
        <td>Amazon (AWS)</td>
        <td><i>In Progress</i></td>
      </tr>
    </tbody>
  </table>

  <br>

  <h2>🛠 Tech Stack</h2>
  <h3>Backend & Infra</h3>
  <p>
    <img src="https://img.shields.io/badge/Spring-87BC00?logo=spring&logoColor=white"> 
    <img src="https://img.shields.io/badge/Java-007396?logo=java&logoColor=white"> 
    <img src="https://img.shields.io/badge/PostgreSQL-4169E1?logo=postgresql&logoColor=white"> 
    <img src="https://img.shields.io/badge/AWS-232F3E?logo=amazonwebservices&logoColor=white">
  </p>

  <h3>Mobile (Experience)</h3>
  <p>
    <img src="https://img.shields.io/badge/Flutter-02569B?logo=flutter&logoColor=white"> 
    <img src="https://img.shields.io/badge/UIKit-F05138?logo=swift&logoColor=white">
  </p>

  <br>

  <h2>📊 Coding Stats</h2>
  <a href="https://solved.ac/crisine">
    <img src="http://mazassumnida.wtf/api/generate_badge?boj=crisine" alt="Solved.ac Profile" />
  </a>
</div>
